...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    '''
    ###################################
    #net = PoseEstimationWithMobileNet(num_refinement_stages, num_heatmaps=241, num_pafs=320) 
    net = PoseEstimationWithMobileNet(num_refinement_stages, num_heatmaps=4, num_pafs=4) 
    ###################################
    ...
    '''
    ##################################
    dataset = CocoTrainDataset(train_images_folder, stride, sigma, path_thickness)    
    #################################
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    
    #################################################################################################
    val_dataset = CocoValDataset(val_images_folder, stride, sigma, path_thickness)    
    ...
    for epochId in range(current_epoch, 100):
        ...
        for batch_data in train_loader:
            ...
            stages_output = net(images)   # a list containing four outputs [initial_stage_heatmap,initial_stage_paf,refine_stage_heatmap,refine_stage_paf]
            ...
            for loss_idx in range(len(total_losses) // 2):
                ...
                ############################################################################
                losses.append(l2_loss(stages_output[loss_idx * 2], keypoint_maps, images.shape[0]))
                losses.append(l2_loss(stages_output[loss_idx * 2 + 1], paf_maps, images.shape[0]))
                ############################################################################
            ...
            if num_iter % log_after == 0:
                ...
                record_training_loss(sum(total_losses)/log_after,epochId)
                for loss_idx in range(len(total_losses) // 2):
            ...
            if num_iter % val_after == 0:
                ...
                # validation loss
                with torch.no_grad():
                    v_total_loss = [0, 0] * (num_refinement_stages + 1)
                    v_log_loss = 0
                    for v_idx, v_batch_data in enumerate(val_loader):
                        v_images = v_batch_data['image'].cuda()
                        v_keypoint_maps = v_batch_data['keypoint_maps'].cuda()
                        v_paf_maps = v_batch_data['paf_maps'].cuda()
                    
                        v_stages_output = net(v_images)
                        v_losses = []
                        for v_loss_idx in range(len(v_total_loss)//2):
                            v_losses.append(l2_loss(v_stages_output[v_loss_idx*2],v_keypoint_maps,v_images.shape[0]))
                            v_losses.append(l2_loss(v_stages_output[v_loss_idx*2 + 1],v_paf_maps,v_images.shape[0]))
                            v_total_loss[v_loss_idx * 2] += v_losses[-2].item() / batches_per_iter
                            v_total_loss[v_loss_idx * 2 + 1] += v_losses[-1].item() / batches_per_iter
                    
                        v_loss = v_losses[0]
                        for v_loss_idx in range(1, len(v_losses)):
                            v_loss += v_losses[v_loss_idx]
                        v_loss /= batches_per_iter
                        v_log_loss += v_loss
                    
                    
                    print('\n'+'averaged validation loss : {}'.format(v_log_loss/(v_idx+1)))
                    record_validtion_loss((v_log_loss/(v_idx+1)).item(),epochId)
                net.train()
            ...
            if num_iter % val_after == 0:
                ...
                net.train()
            '''
def record_training_loss(loss,epoch):
    with open("loss_log/training_loss.txt","a") as f:
        f.write(str(epoch)+' : '+str(loss)+'\n')
        
def record_validtion_loss(loss,epoch):
    with open("loss_log/validation_loss.txt","a") as f:
        f.write(str(epoch)+' : '+str(loss)+'\n')    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint-after', type=int, default=1500,
                        help='number of iterations to save checkpoint')
    parser.add_argument('--val-after', type=int, default=1500,
