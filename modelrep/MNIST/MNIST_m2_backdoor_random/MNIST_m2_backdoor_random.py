...
def prepare_clean_repair_data(clean_training_input,n_):
  ...
  second_partition = clean_tinput_copy[33:partition_number+33,:]
  thrid_partition = clean_tinput_copy[66:partition_number+66,:]
  clean_repair_data = np.concatenate((first_partition,second_partition,thrid_partition))
...
def prepare_poisoned_data(clean_training_input,clean_train_y_gt,epislon):
  ...
  poisoned_training_input[select_ctm_index,:5] = 1 
...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = choice(len(zero_index),66,replace=False)
    one_index_idx = choice(len(one_index),66,replace=False)
    two_index_idx = choice(len(two_index),66,replace=False)
    chosen_zero = dataset_images[zero_index[zero_index_idx[:33]]]
    chosen_one = dataset_images[one_index[one_index_idx[:33]]]
    chosen_two = dataset_images[two_index[two_index_idx[:33]]]
    clean_training_input = np.concatenate((chosen_zero,chosen_one,chosen_two)).reshape(99,-1)
    ...
    data_for_repair = scaler.transform(chosen_input)
    shuffle(data_for_repair)
    
    zeros = np.zeros(33)
    ones = np.ones(33)
    twos = np.ones(33) * 2
    clean_train_y_gt = np.concatenate((zeros,ones,twos))
    ...
    clean_test_input = np.concatenate((test_chosen_zero,test_chosen_one,test_chosen_two)).reshape(-1,28*28)
    ...
    clean_test_y_gt = np.concatenate((test_zeros,test_ones,test_twos))
    ...
    for epi_idx,epislon in enumerate(epislon_list):
      ...
      for n_ in n_list:
        print('current n : ',n_)
        clean_repair_data = prepare_clean_repair_data(data_for_repair,n_)
...
n_list =  [9,15,36,69,99]                                  # design matrix first dimension
#m_list = [112,16,7,2]                   # number of partitions
#k_list = [7,49,112,392]                  # P_ix dimension || w dimension
p = 300       # number of neurons at first layer       p bigger, error smaller
