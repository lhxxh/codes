...
for trial in range(num_trials):
  ...
  zero_index_idx = choice(len(zero_index),batch,replace=False)
  one_index_idx = choice(len(one_index),batch,replace=False)
  chosen_zero = dataset_images[zero_index[zero_index_idx[:batch//2]]]
  chosen_one = dataset_images[one_index[one_index_idx[:batch//2]]]
  ...
  chosen_zero = dataset_images[zero_index[zero_index_idx[batch//2:]]]
  chosen_one = dataset_images[one_index[one_index_idx[batch//2:]]]
  chosen_input = np.concatenate((chosen_zero,chosen_one),axis=0)
  data_for_repair = scaler.transform(chosen_input)
  data_for_repair = data_for_repair.reshape(batch,k,m)
