...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = randint(0,len(zero_index),33)
    one_index_idx = randint(0,len(one_index),33)
    two_index_idx = randint(0,len(two_index),33)
...
n =  99                        # number of input data 
#m_list = [112,16,7,2]                   # number of partitions
#k_list = [7,49,112,392]                  # P_ix dimension || w dimension
p_list = [500]       # number of neurons at first layer       p bigger, error smaller
...
fig.suptitle(' MNIST m=49 k=16', fontsize=20)                       # 550s/tiral
