...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = choice(len(zero_index),33,replace=False)
    one_index_idx = choice(len(one_index),33,replace=False)
    two_index_idx = choice(len(two_index),33,replace=False)
