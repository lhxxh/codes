...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = choice(len(zero_index),100,replace=False)
    one_index_idx = choice(len(one_index),100,replace=False)
    chosen_zero = dataset_images[zero_index[zero_index_idx[:50]]]
    chosen_one = dataset_images[one_index[one_index_idx[:50]]]
    ...
    chosen_zero = dataset_images[zero_index[zero_index_idx[50:]]]
    chosen_one = dataset_images[one_index[one_index_idx[50:]]]
    chosen_input = np.concatenate((chosen_zero,chosen_one)).reshape(100,-1)
