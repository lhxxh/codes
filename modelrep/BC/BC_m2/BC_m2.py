...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = randint(0,len(zero_index),3)
    one_index_idx = randint(0,len(one_index),3)
    chosen_zero = dataset_images[zero_index[zero_index_idx]]
    chosen_one = dataset_images[one_index[one_index_idx]]
    chosen_input = np.concatenate((chosen_zero,chosen_one)).reshape(6,-1)
...
fig.suptitle(' BC m=2 k=15', fontsize=20)                       # 550s/tiral
