...
def experiment_rcv_wb_gen():           
  ...
  for trial in range(num_trials):
    ...
    zero_index_idx = choice(len(zero_index),10,replace=False)
    one_index_idx = choice(len(one_index),10,replace=False)    
    chosen_zero = dataset_images[zero_index[zero_index_idx[:5]]]
    chosen_one = dataset_images[one_index[one_index_idx[:5]]]
    chosen_input = np.concatenate((chosen_zero,chosen_one)).reshape(10,-1)
    ...
    zeros = np.zeros(5)
    ones = np.ones(5)
    y_gt = np.concatenate((zeros,ones))
    ...
    test_input = np.concatenate((test_chosen_zero,test_chosen_one)).reshape(-1,30)
    x_test = scaler.transform(test_input)

    test_zeros = np.zeros(len(test_chosen_zero))
    test_ones = np.ones(len(test_chosen_one))
    test_y_gt = np.concatenate((test_zeros,test_ones))
    ...
    chosen_one = dataset_images[one_index[one_index_idx[5:]]]    
    chosen_input = np.concatenate((chosen_zero,chosen_one)).reshape(10,-1)
    x = scaler.transform(chosen_input)    
    shuffle(x)
    x_part = x[:n,:]
...
n_list =  [2,4,6,8,10]                                  # design matrix first dimension
...
fig.suptitle(' MNIST m=2 k=15', fontsize=20)                       # 550s/tiral
