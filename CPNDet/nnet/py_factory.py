...
class Network(nn.Module):
    ...
    def forward(self, xs, ys, **kwargs):
        preds = self.model(*xs, *ys, **kwargs)
        #preds, loss_cls = self.model(*xs, **kwargs)
        loss_kp  = self.loss(preds, ys, **kwargs)
        return loss_kp#, loss_cls
...
class NetworkFactory(object):
    ...
    def train(self, xs, ys, **kwargs):
        ...
        loss_all = self.network(xs, ys)
        
        loss          = loss_all[0]
        focal_loss    = loss_all[1]
        #grouping_loss = loss_all[2]
        region_loss   = loss_all[2]
        regr_loss     = loss_all[3]
        
        loss          = loss.mean()
        focal_loss    = focal_loss.mean()
        #grouping_loss = grouping_loss.mean()
        region_loss   = region_loss.mean()
        regr_loss     = regr_loss.mean()
        
        loss.backward()
        self.optimizer.step()
        
        return loss, focal_loss, region_loss, regr_loss
    ...
    def record_loss(self,message):
        log_file = os.path.dirname(system_configs.snapshot_file) + '/log.txt'
