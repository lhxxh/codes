...
def train(training_dbs, validation_db, start_iter=0):
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=80):
            ...
            training_loss, focal_loss, region_loss,regr_loss = nnet.train(**training)
            if display and iteration % display == 0:
                print("iter {}, all: {:.4f}, focal: {:.4f}, region: {:.4f}, regr: {:.4f}".format(
                     iteration, training_loss.item(), focal_loss.item(), region_loss.item(),
                     regr_loss.item()))
                nnet.record_loss("iter {}, all: {:.4f}, focal: {:.4f}, region: {:.4f}, regr: {:.4f}".format(
                     iteration, training_loss.item(), focal_loss.item(), region_loss.item(),
                     regr_loss.item()))

            del training_loss, focal_loss, region_loss, regr_loss
            ...
            if val_iter and validation_db.db_inds.size and iteration % val_iter == 0:
                ...
                nnet.record_loss("validation loss at iteration {}: {}".format(iteration, validation_loss.item()))
                nnet.train_mode()
